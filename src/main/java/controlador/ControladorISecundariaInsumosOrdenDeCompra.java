...
public class ControladorISecundariaInsumosOrdenDeCompra {
...
//    private JFXButton btnNuevo;
    ...
    private ObservableList<InsumoFX> obListDetalleOrdenDeCompra = FXCollections.observableArrayList(); //para la tabla detalle factura
    ...
    public ControladorISecundariaInsumosOrdenDeCompra() {
...
//    	setearOrdenDatosEnTabla();	////////!!!!!!!!!!!!!!!!! ahora lo llamo desde controlador ingresofactura
    ...
    public void cancelar() {
    	...
    	//x el cargar insumos
//    	ControladorICsd_Principal.controllerIVistaOrdenDeCompra.setObListInsumosPantSecundaria(this.getObListInsumos());
    	
    	//le digo al contenedor ppal q se cierre (controlador iPpalSecundariaInsumos)
    	ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getControllerPpalSecundariaInsumos().getOtroStage().close();
    }
  ...
  //borra la ultima fila agregada a la tabla coincidencias,es decir, borra el ultimo insumo agregado
    ...
    void deshacer(ActionEvent event) {
  		obListDetalleOrdenDeCompra.remove(obListDetalleOrdenDeCompra.size() - 1);
  	...
  	//a la verdadera tabla detalle orden de compra (vistaOrdenDeCompra)
  	@FXML
    void guardar(ActionEvent event) {
  		try {
  			ObservableList<DetalleOrdenDeCompraFX> obListDetalleOrden = FXCollections.observableArrayList();
  			
  			for (InsumoFX insumoFX : obListDetalleOrdenDeCompra) {
  				DetalleOrdenDeCompraFX detalleOrdenDeCompraFX = new DetalleOrdenDeCompraFX();
  				
  				detalleOrdenDeCompraFX.nombreInsumo.set(insumoFX.nombreInsumo.get());
  				detalleOrdenDeCompraFX.articulo.set(insumoFX.articulo.get());
  				if (insumoFX.nroPedido.get() != null) {
  					detalleOrdenDeCompraFX.cantidad.set(insumoFX.nroPedido.get());
				} else {
					detalleOrdenDeCompraFX.cantidad.set(0);
				}
  				detalleOrdenDeCompraFX.unidades.set(0);
  				detalleOrdenDeCompraFX.nombreCategoria.set(insumoFX.nombreCategoria.get());
  				detalleOrdenDeCompraFX.idCategoria.set(insumoFX.idCategoria);
  				detalleOrdenDeCompraFX.nombreSector.set(insumoFX.nombreSector.get());
  				
  				detalleOrdenDeCompraFX.referencia.set(insumoFX.referencia.get());
  				
  				obListDetalleOrden.add(detalleOrdenDeCompraFX);
  			}
  			

  		    //1ero tengo q verificar si la tabla detalle orden de compra esta vacia
  			//si es asi, a su obList le seteo la de este controlador
  			if (ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getObListDetalleOrdenDeCompra().isEmpty()) {
  				
  				ControladorICsd_Principal.controllerIVistaOrdenDeCompra.setObListDetalleOrdenDeCompra(obListDetalleOrden);
  			
  			} else { 
  				for (DetalleOrdenDeCompraFX docFX : obListDetalleOrden) {
  					ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getObListDetalleOrdenDeCompra().add(docFX);
  				}
  			}
  			
  			ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getDetalleOrdenDeCompra().
  			setItems(ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getObListDetalleOrdenDeCompra());
  	    	...
  	    	ControladorICsd_Principal.controllerIVistaOrdenDeCompra.habilitarBotones();
...
//	}
	...
	public void llenarTablaInsumos() {	
		if (ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getObListInsumosPantSecundaria().isEmpty()) {
			cargarInsumos();
//			setearColNombreSector();  ahora el nombre sector lo obtengo dentro del query en cargarInsumos() -- CRUD.obtenerListaInsumosActivosAgrupadosPorArticulo()
			...
			for (InsumoFX insumoFX : ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getObListInsumosPantSecundaria()) {
...
//		pintarFilas();
	...
	private void cargarInsumos() {
		...
		try {
			...
			List<Insumo> listaInsumos = CRUD.obtenerListaInsumosActivosAgrupadosPorNombreConStockGeneral();  
...
//			Integer idInsumoBD = CRUD.obtenerIdInsumoPrecioActual(rowData.nombreInsumo.get(), rowData.articulo.get(), rowData.referencia.get());
			...
			Integer idInsumoBD = CRUD.obtenerIdInsumoSucursalDiagnos(rowData.nombreInsumo.get(), rowData.articulo.get(), rowData.referencia.get());
	...
	//el argumento "tipo", me indica si el insumoIn, es uno recien creado o si es un insumo q ya estaba cargado
  	public void agregarInsumoAFila(Insumo insumoIN, String tipo, String nombreSector) {
  		...
  		meterInsumoADetalleOrdenDeCompra(insumoIN, nombreSector);
  		...
  		if (tipo.equals("InsumoNuevo")) { //solamente ingresa al if, si viene desde el alta insumo
  			...
  			ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getiPpalSecundaria().
  																setCenter(ControladorICsd_Principal.controllerIVistaOrdenDeCompra.getiSecundariaInsumos());
  	...
  	private void meterInsumoADetalleOrdenDeCompra(Insumo insumoIN, String nombreSectorIN) {
  		InsumoFX insFX = new InsumoFX(insumoIN);
  		insFX.nombreSector.set(nombreSectorIN);
  		obListDetalleOrdenDeCompra.add(insFX);
  		detalleOrdenDeCompra.setItems(obListDetalleOrdenDeCompra);
	}
  ...
  //metodo q es invocado desde el alta insumo, cuando se apreta en "cancelar"
	...
	public void volver() {
		...
		//reseteo el objeto de referencia a la pantalla a la cual volver
		ControladorICsd_Principal.controllerAltaInsumo.setIngresoOrdenDeCompra(false);
	}
